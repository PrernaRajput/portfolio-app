...
export default function ThemeToggle () {
    ...
    return (
        ...
        ${isDark ? 'bg-ashGray border-ashGray' : 'bg-coolGray border-coolGray'}`}
        >
            <div
                className={`w-6 h-6 rounded-full shadow-md flex items-center justify-center text-xs transition-transform duration-300
          transform ${isDark ? 'translate-x-6 bg-gunmetal text-ashGray' : 'translate-x-0 bg-white text-coolGray'}`}
            >
                {isDark ? <FaMoon /> : <FaSun />}
            </div>
        </button>
